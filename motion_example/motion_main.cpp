...
static void draw_line(my_image_comp *ref, my_image_comp *tgt, mvector vec,
	...
	if (vec.x == 0 && vec.y > 0) {
		for (int index = 0; index <= vec.y; ++index) {
			center[index * stride] = 0;
		}
	}
	else if (vec.x == 0 && vec.y < 0) {
		for (int index = vec.y; index <= 0; ++index) {
			center[index * stride] = 0;
		}
	}

	if (vec.y == 0 && vec.x > 0) {
		for (int index = 0; index <= vec.x; ++index) {
			center[index] = 0;
		}
	}
	else if (vec.y == 0 && vec.x < 0) {
		for (int index = vec.x; index <= 0; ++index) {
			center[index] = 0;
		}
	}
	...
	if (vec.y > 0 && vec.x > 0) {
		if (vec.y >= vec.x) {
			int block_each_x = (vec.y + vec.x - 1) / vec.x;
			center[vec.y * stride + vec.x] = 0;
			for (c = 0, r = 0; c < vec.x; ++c) {
			...
			int block_each_y = (vec.x + vec.y - 1) / vec.y;
			center[vec.y * stride + vec.x] = 0;
			for (r = 0, c = 0; r < vec.y; ++r) {
