	   ...
	   absolute differences (SAD) metric. */
{
	...
	for (vec.y = -8; vec.y <= 8; vec.y+=4)
		for (vec.x = -8; vec.x <= 8; vec.x+=4)
		{
			int ref_row = ((float)pixel)*((float)start_row - vec.y);
			int ref_col = ((float)pixel)*((float)start_col - vec.x);
			if ((ref_row < 0) || (ref_col < 0) ||
	...
	mvector temp_vec = best_vec;
	for (vec.y = temp_vec.y - 2; vec.y <= temp_vec.y + 2; vec.y += resolution)
		for (vec.x = temp_vec.x - 2; vec.x <= temp_vec.x + 2; vec.x += resolution)
		{
			int ref_row = ((float)pixel)*((float)start_row - vec.y);
			int ref_col = ((float)pixel)*((float)start_col - vec.x);
			if ((ref_row < 0) || (ref_col < 0) ||
