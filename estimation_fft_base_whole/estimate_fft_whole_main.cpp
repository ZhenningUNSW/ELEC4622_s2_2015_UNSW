...
void my_aligned_image_comp::extract(int location_x, int location_y, my_aligned_image_comp * in)
{
	int half_outsize = width / 2;
		for (int i = -half_outsize; i < half_outsize; ++i)
			for (int j = -half_outsize; j < half_outsize; ++j) {
				this->buf[(i + half_outsize) * this->stride + (j + half_outsize)]
					= in->buf[i * in->stride + j + location_x + location_y * in->stride];
			}
}
...
main(int argc, char *argv[])
{
	if (argc != 5)
	{
		fprintf(stderr, "Usage: %s <in bmp file> <out bmp file> <N> <alpha>\n", argv[0]);
		return -1;
	}
	...
	try {
		...
		my_aligned_image_comp *output_comps =
			new my_aligned_image_comp[3];
		
		int out_comps = 3;

		for (int n = 0; n < out_comps; ++n)
			output_comps[n].init(height, width, 0);

		my_aligned_image_comp *intermedia_comps =
			new my_aligned_image_comp[num_comps];
		for (n = 0; n < num_comps; n++) {
			intermedia_comps[n].init(out_size, out_size, 0);// Don't need a border for output
		}
		...
		for (int y = 0; y < height; ++y)
			for (int x = 0; x < width; ++x){
				for (n = 0; n < num_comps; n++) {
					...
					intermedia_comps[n].extract(x, y, input_comps + n);
					intermedia_comps[n].mean_substract();
					intermedia_comps[n].hanning_window();
					//output_comps[n].perform_dft(alpha);
					intermedia_comps[n].perform_fft(alpha);
					float table[3] = {0,0,0};
					intermedia_comps[n].calculate_estimation(table);

					for (int z = 0; z < out_comps; ++z){
						output_comps[z].buf[y * output_comps[z].stride + x] = table[z];
					}
				}
			}
		...
		bmp_out out;
		io_byte *out_line = new io_byte[width*out_comps];
		if ((err_code = bmp_out__open(&out, argv[2], width, height, out_comps)) != 0)
			throw err_code;
		for (r = height - 1; r >= 0; r--)
		{ // "r" holds the true row index we are writing, since the image is
		  // written upside down in BMP files.
			for (n = 0; n < out_comps; n++)
			{
				io_byte *dst = out_line + n; // Points to first sample of component n
				float *src = output_comps[n].buf + r * output_comps[n].stride;
				for (int c = 0; c < width; c++, dst += out_comps) {
