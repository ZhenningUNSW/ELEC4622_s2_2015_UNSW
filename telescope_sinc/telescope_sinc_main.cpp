...
static mvector
find_motion_mse_base(my_image_comp *ref, my_image_comp *tgt,
...
static void interpolation(my_image_comp *in, my_image_comp *out) {
	...
	int filter_tap = 4;
	//signed 32-bit fix point table, friction 15-bit
	int sinc_kernal[9] = {0,-1,7,-20,79,79,-20,7,-1};
	int *kernal = sinc_kernal + filter_tap;

	my_image_comp *temp_image = new my_image_comp;
	temp_image->init(out->height, in->width, 4);

	for (int r = 0; r < out->height; ++r)
		for (int c = 0; c < in->width; ++ c) {
			if ((r % 2) == 0) {
				int d = r >> 1;
				temp_image->buf[r * temp_image->stride + c] = in->buf[d * in->stride + c];
			}
			else {
				int sum = 0;
				int d = r >> 1;
				int *ip = in->buf + (d * in->stride) + c;
				int *op = temp_image->buf + r * temp_image->stride + c;
				for (int k = -filter_tap; k <= filter_tap; ++k) {
					sum += ip[k * in->stride] * kernal[k];
				}
				sum = sum >> 7;
				*op = sum;
			}
	...
	temp_image->perform_boundary_extension();

	for (int r = 0; r < out->height; ++r)
		for (int c = 0; c < out->width; ++c) {
			if (c % 2 == 0) {
				int d = c >> 1;
				out->buf[r * out->stride + c] = temp_image->buf[r * temp_image->stride + d];
			}
			else {
				int d = c >> 1;
				int sum = 0;
				int *op = out->buf + (r * out->stride) + c;
				int *ip = temp_image->buf + r * temp_image->stride + d;
				for (int k = -filter_tap; k <= filter_tap; ++k) {
					sum += ip[k] * kernal[k];
				}
				sum = sum >> 7;
...
/*
	...
	fprintf_s(fp, "mypic = uint8([");
	for (int r = 0; r < out->height; ++r) {
		for (int c = 0; c < out->width; ++c) {
			fprintf_s(fp, "%d, ",
				out->buf[r * out->stride + c]);
		}
		fprintf_s(fp, ";\n");
	}
	fprintf_s(fp, "]);");
...
#else
				mvector vec = find_motion_mse_base(&(ref_frame), &(mono[1]),
					r, c, block_width, block_height, 4);
#endif
